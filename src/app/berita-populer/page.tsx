...
export const metadata = { title: "Berita Populer" };

const PAGE_SIZE = 12;

export default async function BeritaPopulerPage({
  searchParams,
}: {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8">
      <h1 className="mb-6 text-2xl font-bold text-ink md:text-3xl">Berita Populer</h1>

      {(!articles || articles.length === 0) && (
        <p className="text-sm text-ink-muted">Belum ada berita populer.</p>
      )}
      ...
      {totalPages > 1 && (
        <div className="mt-8 flex items-center justify-center gap-2">
          {page > 1 && (
            <Link
              href={`/berita-populer?page=${page - 1}`}
